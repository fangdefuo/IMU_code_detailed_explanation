...
void AHRS_EKF_Init()
{
    ...
    hx=pAHRS_Data_filtered->magX;
    hy =pAHRS_Data_filtered->magY;
  int i,j;
  
  psi = -atan2(-hy, -hx);//航向角计算
  for (i=0; i<7; i++)
    x_e[i][0] = 0.0;
  //计算航向角正余弦值
  x_e[0][0]= cos(psi*0.5);
...
{    //GPIO_SetBits(GPIOC, GPIO_Pin_6); 
     /////////////////p1-1
      ...
      dt = 0.02; 
      Hdt = 0.5*dt;      
      pc = ((pAHRS_Data_filtered->RollRate)  - x_e[4][0]) * Hdt;  /////?????角度误差
      qc = ((pAHRS_Data_filtered->PitchRate) - x_e[5][0]) * Hdt;
      rc = ((pAHRS_Data_filtered->YawRate)   - x_e[6][0]) * Hdt;  
      ...
      dTemp[0] =  x_e[1][0]* Hdt;  F[0][4]=dTemp[0];/////角度
      ...
      for(i=0;i<7;i++)
      {
        for(j=0;j<7;j++)
        { 
          F1[i][j]=F[j][i];//F1为F的转秩，卡尔曼滤波公式中有
        }
      }
      ///用 mid和F1可以更新P
        float mid[7][7]={0};
        ...
        for(i=0;i<4;i++)
	{
          dTemp[0] = P[i][i] + Q1;///计算出带有噪声的P
          P[i][i]=dTemp[0];
	}
        for(i=4;i<7;i++)
	{
          dTemp[0] = P[i][i];
          P[i][i]=dTemp[0]+Q2;///////计算出带有噪声的P
	}
        ...
        float ax = pAHRS_Data_filtered->Xacc;
	float ay = pAHRS_Data_filtered->Yacc;
	float az = pAHRS_Data_filtered->Zacc;
        ///////////////p1-2
        ...
        {//p2-1
          ...
          /////////////////计算 mid3------->midd--------->K,K才是最终目标，其他是中间变量////////////////////////////////
          float mid3[3][3]={0.0};          
          for(i=0;i<3;i++)
          {
            for(j=0;j<3;j++)
 ...
 //////////////////////////////////////////////***************************/////////////////////////////////////
        ...
        {//p3-1
          float calibrated_hx = pAHRS_Data_filtered->magX; //这两个变量可以用于计算航向角
          float calibrated_hy = pAHRS_Data_filtered->magY;
          cPHI= cos(pAHRS_Data_filtered->Roll);
          sPHI= sin(pAHRS_Data_filtered->Roll);
          Bxc = calibrated_hx * cos(pAHRS_Data_filtered->Pitch) + (calibrated_hy * sPHI + pAHRS_Data_filtered->magZ * cPHI) * sin(pAHRS_Data_filtered->Pitch);
          Byc = calibrated_hy * cPHI - pAHRS_Data_filtered->magZ * sPHI;
          
          //将x_e归一化
          norm = 1.0/sqrt(x_e[0][0]*x_e[0][0]+x_e[1][0]*x_e[1][0]+x_e[2][0]*x_e[2][0]+x_e[3][0]*x_e[3][0]);
          for(i=0;i<4;i++)
          {
            dTemp[0] = x_e[i][0]*norm;//归一化
            x_e[i][0]=dTemp[0];
          }
        ...
        ////////////计算出滤波后的角度
        double temp_asinvalue = -2*(x_e[1][0]*x_e[3][0]-x_e[0][0]*x_e[2][0]);
        if(temp_asinvalue > 1.0)
        temp_asinvalue = 1.0;
        if(temp_asinvalue < -1.0)
        temp_asinvalue=-1.0;
        pAHRS_Data_filtered->Pitch = asin(temp_asinvalue);//asin(-2*(x_e[1][0]*x_e[3][0]-x_e[0][0]*x_e[2][0]));
        pAHRS_Data_filtered->Roll  = atan2(2*(x_e[0][0]*x_e[1][0]+x_e[2][0]*x_e[3][0]),1-2*(x_e[1][0]*x_e[1][0]+x_e[2][0]*x_e[2][0]));
        pAHRS_Data_filtered->Yaw   = atan2(2*(x_e[1][0]*x_e[2][0]+x_e[0][0]*x_e[3][0]),1-2*(x_e[2][0]*x_e[2][0]+x_e[3][0]*x_e[3][0]));
        Compensate_Zacc=pAHRS_Data_filtered->Zacc/(cos(pAHRS_Data_filtered->Roll)*cos(pAHRS_Data_filtered->Pitch));
        //Yaw_Recalculate();
        //p4-2
       //GPIO_ResetBits(GPIOC, GPIO_Pin_6);
        YAW++;//=pAHRS_Data_filtered->Yaw*57.3;
}
